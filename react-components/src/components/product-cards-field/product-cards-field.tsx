...
export default class ProductCardsField extends React.Component<{
  productsProps: IProductCardProps[];
}> {
  ...
  private static addCards(productsProps: IProductCardProps[]): JSX.Element[] {
    return productsProps.map((productProps) => (
      <ProductCard key={productProps.title} productProps={productProps} />
    ));
  }
  ...
  render(): JSX.Element {
    ...
    return <div className="product-cards-field">{cards}</div>;
  }
}
