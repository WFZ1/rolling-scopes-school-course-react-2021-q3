...
export default class ProductCard extends React.Component<{ productProps: IProductCardProps }> {
  ...
  private addProductSizes(selectedColor: string | undefined): JSX.Element[] {
    ...
    return sizes.map((size) => {
      ...
      return <span key={ size.value } className={ classes } { ...style }>{ size.value }</span>;
    });
  }
  ...
  private addProductColors(): JSX.Element[] {
    ...
    return colors.map((color) => {
      ...
      return <span key={ color.hex } className={ classes } style={{ backgroundColor: color.hex }}></span>;
    });
  }
    ...
    return (
      <div className="product-card" data-brand={ product.brand }>
        ...
        <div className="product-card__info">
          ...
          <button className="product-card__btn" type="button">Buy Now</button>
        </div>
        <span className="product-card__figure" style={{ backgroundColor: selectedColor?.hex }}></span>
      </div>
    );
  }
}
